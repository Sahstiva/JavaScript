...
function num2obj(n) {
    if (n >= 0 && n < 1000) {
        this.hundreds = (n - n % 100) / 100;
        this.tens = ((n - n % 10) - (n - n % 100)) / 10;
        this.ones = n % 10;
    }
    else
...
function createObj(n) {
    document.getElementById('PutTextHere').innerHTML = "&nbsp;";
    let newObj = new num2obj(n);
    for (let i of Object.keys(newObj))
        document.getElementById('PutTextHere').innerHTML += ":" + newObj[i];
}
function addToBasketObj() {
    if (!BasketObj) {
        BasketObj = new Basket();
    }
    newItem = new ItemObj((Math.random() * 100).toFixed(2));
    BasketObj.addItem(newItem);
    document.getElementById('Basket').innerHTML += BasketObj.items[BasketObj.length - 1].name + "/" + BasketObj.items[BasketObj.length - 1].price + "; ";
    document.getElementById('BasketSum').innerHTML = "Сумма: " + BasketObj.sum.toFixed(2);
}
function ItemObj(itemPrice) {
    this.price = itemPrice;
    this.name = "Товар " + BasketObj.length;
}
function Basket() {
    this.length = 0;
    this.sum = 0;
    ...
    this.addItem = function (newItem) {
        ...
        this.sum += parseFloat(newItem.price);
    }
}
